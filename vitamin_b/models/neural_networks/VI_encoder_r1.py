...
class VariationalAutoencoder(object):
    ...
    def __init__(self, name, n_input=256, n_output=4, n_channels=3, n_weights=2048, n_modes=2, n_hlayers=2, drate=0.2, n_filters=8, filter_size=8, maxpool=4, n_conv=2, strides=1, dilations=1, batch_norm=True, twod_conv=False, parallel_conv=False):
        ...
        self.nonlinearity = tf.nn.relu #tf.keras.activations.swish #tf.nn.relu
        ...
        with tf.name_scope("VI_encoder_r1"):
            ...
            if self.n_conv>0:
                if self.twod_conv:
                    ...
                    #conv_pool_t = tf.slice(conv_pool_t_orig,[0,0,0,0],[-1,-1,-1,1])
                    #for j in range(1,self.n_channels):
                    #    conv_pool_t = tf.concat([conv_pool_t,tf.slice(conv_pool_t_orig,[0,0,0,j],[-1,-1,-1,1])],axis=0)
                    conv_padding = 'SAME'
                ...
                #    conv_pool_t = tf.slice(conv_pool_t_orig,[0,0,0,j],[-1,-1,-1,1])
                for i in range(self.n_conv):
                    ...
                    conv_padding = 'SAME'
                    #if j==0:
                    #    conv_pool_t_end = conv_pool_t
                    #else:
                    #    conv_pool_t_end = tf.concat([conv_pool_t_end,conv_pool_t],axis=3)
    ...
    def _create_weights(self):
        ...
        with tf.variable_scope("VI_encoder_r1"):            
            ...
            if self.n_conv>0:
                ...
                for i in range(self.n_conv):
                    ...
                    if self.twod_conv:
                        ...
                        temp = tf.reshape(vae_utils.xavier_init(self.filter_size[i], dummy_t*self.n_filters[i]),[self.filter_size[i], 1, dummy_t, self.n_filters[i]])
                        all_weights['VI_encoder_r1'][weight_name+'t'] = tf.Variable(initial_value=temp, trainable=True, dtype=tf.float32)
            ...
            all_weights['VI_encoder_r1']['b_scale_diag'] = tf.Variable(tf.random.uniform([self.n_output*self.n_modes], -4, 0., dtype=tf.float32),dtype=tf.float32)
