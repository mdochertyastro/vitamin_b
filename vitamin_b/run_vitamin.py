...
def gen_samples(params=params,bounds=bounds,fixed_vals=fixed_vals,model_loc='model_ex/model.ckpt',test_set='test_waveforms/',num_samples=None,plot_corner=True,use_gpu=False,save_vit=False):
    ...
    def progress(count, total, suffix=''): # function to print progress bar in cmd prompt
        ...
        sys.stdout.flush()  # As suggested by Rom Ru
    ...
    norm_samples = np.zeros((num_timeseries,num_samples,len(params['inf_pars']))) # want to keep the more complex samples shape to allow for more test waveforms
    vit_samples=np.zeros_like(norm_samples)
    for i in range(num_timeseries):
        norm_samples[i,:], dt, _  = CVAE_model.run(params, np.expand_dims(y_data_test[i],axis=0), len(params['inf_pars']),
                                                              params['y_normscale'],
                                                              model_loc)
        ...
        for q_idx,q in enumerate(params['inf_pars']):
            par_min = q + '_min'
            par_max = q + '_max'
            vit_samples[i,:,q_idx] = (norm_samples[i,:,q_idx] * (bounds[par_max] - bounds[par_min])) + bounds[par_min]

        # Convert hour angle back to RA
        vit_samples[i,:] = convert_ra_to_hour_angle(vit_samples[i,:], params, rand_pars=False)
        ...
        if plot_corner==True:
            ...
            defaults_kwargs = dict(
                    ...
                    plot_density=False, plot_datapoints=True,
                    max_n_ticks=3)
            figure = corner.corner(vit_samples[i,:,:],**defaults_kwargs,labels=parnames)
